...
def website_registration(request):
	...
	if (form.is_valid()):
		...
		bodyText = "Swap: Three friends with iPhones are needed to activate Swap. Use this personalized link to invite others: " + link
...
def send_notifications_test(request):
	...
	if data.has_key('photo_id'):
		customPayload['photo_id'] = int(data['photo_id'])

	if data.has_key('strand_id'):
		customPayload['strand_id'] = int(data['strand_id'])		
