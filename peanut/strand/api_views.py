...
def actions_list(request):
	...
	if (form.is_valid()):
		...
		actions = Action.objects.prefetch_related('user', 'strand').exclude(user=user).filter(Q(action_type=constants.ACTION_TYPE_FAVORITE) | Q(action_type=constants.ACTION_TYPE_ADD_PHOTOS_TO_STRAND) | Q(action_type=constants.ACTION_TYPE_COMMENT)).filter(Q(photo_id__in=photoIds) & Q(strand_id__in=strandIds)).order_by("-added")[:40]
